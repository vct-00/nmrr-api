...
const getBookingByRoomId = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        res
            .status(200)
...
const addBooking = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (booking.endDate < booking.startDate)
            throw Error("Time invalid");
        //check if room schedule is available
        let conflictBookings = (yield booking_1.default.find({ roomId: booking.roomId })).filter((x) => (x.startDate <= booking.startDate && x.endDate >= booking.startDate) ||
            (x.startDate <= booking.endDate && x.endDate >= booking.endDate));
        if (conflictBookings.length != 0)
            throw Error("Room Schedule unavailable.");
        //check if room schedule is available
        conflictBookings = (yield booking_1.default.find({ userId: booking.userId })).filter((x) => (x.startDate <= booking.startDate && x.endDate >= booking.startDate) ||
            (x.startDate <= booking.endDate && x.endDate >= booking.endDate));
        if (conflictBookings.length != 0)
...
const getBookingByUserId = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        res.status(200).json(yield booking_1.default.find({ userId: decoded._id }));
    }
    catch (err) {
        return res.status(500).json({ message: err.message });
    }
